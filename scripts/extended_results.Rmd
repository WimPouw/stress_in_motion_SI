---
title: "Extended Results: Foreign Language Learners Show a Kinematic Accent in Their Co-speech Hand Movements"
description: | 
  Extended Results
author:
- name: Hans Rutger Bosker
  ...
  orcid_id: 0000-0002-2628-7738
- name: Marieke Hoetjes
  email: marieke.hoetjes@ru.nl
  url: https://www.mariekehoetjes.nl/
  affiliation: Centre for Language Studies, Radboud University
  affiliation_url: https://www.ru.nl/
- name: Doenja Hustin
  email: doenja.hustin@ru.nl
  affiliation: Radboud University
  affiliation_url: https://www.ru.nl/
- name: Wim Pouw
  email: w.pouw@tilburguniversity.edu
  url: https://wimpouw.com/
  affiliation: Department of Computational Cognitive Science, Research Center for Cognitive Science & Artificial Intelligence, Tilburg University; Donders Institute for Brain, Cognition and Behaviour, Radboud University
  affiliation_url: https://www.tilburguniversity.edu/
  orcid_id: 0000-0003-2729-6502
- name: Lieke van Maastricht
  email: lieke.vanmaastricht@ru.nl
  affiliation: Centre for Language Studies, Radboud University
  affiliation_url: https://www.ru.nl/cls/
date: "`r Sys.Date()`"
output:
...
# Generate bibliography for all loaded packages plus base R
knitr::write_bib(c(.packages(), 
                   'ggplot2', 'plotly', 'RColorBrewer', 'wesanderson', 
                   'ggbeeswarm', 'papaja', 'nlme', 'plyr', 'lsmeans', 
                   'tidyr', 'lme4', 'simr', 'gridExtra', 'stringr', 
                   'readr', 'cowplot', 'base'), 
                 "packages.bib")
```
...
D$stressdiff_cat <- ifelse(D$stressed.syllable.L1>D$stressed.syllable.L2, 'L1 follows', 
                           ifelse(D$stressed.syllable.L1<D$stressed.syllable.L2, 'L1 precedes', 'L1_L2 match'))
...
Table 1. Percentage of accurate stress placement trials in stress-matching items.
...
Table 2. Percentage of accurate stress placement trials in stress-mismatching items.
...
```{r, echo = FALSE, fig.cap="Percentage of trials with correct stress placement."}
...
```{r, echo = FALSE, message= FALSE, warning = FALSE, fig.width = 7, fig.cap="Effect of gesture vs. no gesture on acoustic markers of stress (also only boxplots for visibility"}
a <- ggplot(D, aes(x = condition, y = D$peakF0z, color = condition)) + geom_quasirandom(alpha=0.2) + 
  geom_boxplot(alpha = 0, color = "black") + scale_color_manual(values=wes_palette(n=2, name = "Royal1"))+ylab("peak F0 (normalized)")+theme_bw()+theme(legend.position = "none")+ theme(axis.text.x=element_text(angle = -70, hjust = 0))
b <- ggplot(D, aes(x = condition, y = D$peakAMPz, color = condition)) + geom_quasirandom(alpha=0.2) + 
  geom_boxplot(alpha = 0, color = "black") + scale_color_manual(values=wes_palette(n=2, name = "Royal1"))+ylab("peak envelope (normalized)")+theme_bw()+theme(legend.position = "none")+ theme(axis.text.x=element_text(angle = -70, hjust = 0))
c <- ggplot(D, aes(x = condition, y = D$sDURz, color = condition)) + geom_quasirandom(alpha=0.2) + 
  geom_boxplot(alpha = 0, color = "black") + scale_color_manual(values=wes_palette(n=2, name = "Royal1"))+ylab("vocal duration (normalized)")+theme_bw()+theme(legend.position = "none")+ theme(axis.text.x=element_text(angle = -70, hjust = 0))
d <- ggplot(D, aes(x = condition, y = stressSCORE, color = condition)) + geom_quasirandom(alpha=0.2) + 
  geom_boxplot(alpha = 0, color = "black") + scale_color_manual(values=wes_palette(n=2, name = "Royal1"))+ylab("stress score (normalized)")+theme_bw()+theme(legend.position = "none")+ theme(axis.text.x=element_text(angle = -70, hjust = 0))

subplot(ggplotly(a), ggplotly(b), ggplotly(c), ggplotly(d), titleY = TRUE,margin = 0.04)

a <- ggplot(D, aes(x = condition, y = D$peakF0z, color = condition))  + geom_boxplot(alpha = 0, color = "black") + scale_color_manual(values=wes_palette(n=2, name = "Royal1"))+ylab("peak F0 (normalized)")+
  theme_bw()+theme(legend.position = "none")+ theme(axis.text.x=element_text(angle = -70, hjust = 0))
b <- ggplot(D, aes(x = condition, y = D$peakAMPz, color = condition))  + geom_boxplot(alpha = 0, color = "black") + scale_color_manual(values=wes_palette(n=2, name = "Royal1"))+ylab("peak envelope (normalized)")+
  theme_bw()+theme(legend.position = "none")+ theme(axis.text.x=element_text(angle = -70, hjust = 0))
c <- ggplot(D, aes(x = condition, y = D$sDURz, color = condition)) + geom_boxplot(alpha = 0, color = "black") + scale_color_manual(values=wes_palette(n=2, name = "Royal1"))+ylab("vocal duration (normalized)")+
  theme_bw()+theme(legend.position = "none")+ theme(axis.text.x=element_text(angle = -70, hjust = 0))
d <- ggplot(D, aes(x = condition, y = stressSCORE, color = condition)) + geom_boxplot(alpha = 0, color = "black") + scale_color_manual(values=wes_palette(n=2, name = "Royal1"))+ylab("stress score (normalized)")+
  theme_bw()+theme(legend.position = "none")+ theme(axis.text.x=element_text(angle = -70, hjust = 0))
...
model0 <- lme(acoust_out~1, data = Dlong, random =  list(~1|ppn, ~1|target), 
...
model1 <- lme(acoust_out~marker*condition, data = Dlong, random =  list(~1|ppn), 
              method = "ML", na.action = na.exclude)
model2 <- lme(acoust_out~marker*condition*accuracy_devcod, data = Dlong, random =  list(~1|ppn), 
              method = "ML", na.action = na.exclude)
...
```{r, echo = FALSE, fig.cap="Gesture-speech (a)synchrony depending on stress-(mis)match and stress mark"}
subD <- subset(D, condition == "gesture")
a <- ggplot(subD, aes(x= asynchrony, color = stress)) + geom_density(size= 2)+
  theme_bw()+ylim(1e-12, NA)+facet_grid(.~accent)
a <- a + scale_color_manual(values=wes_palette(n=2, name = "BottleRocket2")) + 
  ggtitle("asynchrony as a function of stress difference and accent")
ggplotly(a)
```
...
```{r, echo = F, fig.cap="Directional gesture-speech (a)synchrony in stress-mismatching and stress-matching items."}
#overall asynchrony in L1 direction (left=L1)
Dsub <- subset(D, (condition == "gesture") & (correct == "L2 correct"))
a <- ggplot(Dsub, aes(x = stress, y= asynchrony_L2L1))+
  geom_hline(yintercept = 0, color = "red", size = 0.5)+geom_violin(fill=NA) + 
  geom_quasirandom(color = "black", size = 0.7, alpha = 0.5)+geom_boxplot(alpha = 0)+theme_bw() + coord_flip()
...
```{r, echo = T, fig.cap="Directional gesture-speech (a)synchrony in stress-mismatching and stress-matching items."}
...
model0 <- lme(asynchrony_L2L1~1, data = Dsub, random = list(~1|ppn, ~1|target), 
              method = "ML", na.action = na.exclude)

model1 <- lme(asynchrony_L2L1~stress, data = Dsub, random =  list(~1|ppn, ~1|target), 
              method = "ML", na.action = na.exclude)
...
model0 <- lme(asynchrony_L2L1~1, data = Dsub_mismatching, random = list(~1|ppn, ~1|target), 
              method = "ML", na.action = na.exclude)

model1 <- lme(asynchrony_L2L1~distance, data = Dsub_mismatching, random =  list(~1|ppn, ~1|target), 
              method = "ML", na.action = na.exclude)
print(anova(model0, model1)) #test difference basemodel versus model 1
summary(model1) 
```
...
```{r, echo=F, fig.cap= "Raw gesture-speech (a)synchrony in stress-mismatching and stress-matching items."}
#splitted
...
```{r}
model0 <- lme(asynchrony~1, data = Dsub, random =  list(~1|ppn, ~1|target), 
              method = "ML", na.action = na.exclude)
...
model3 <- lme(asynchrony~stressdiff_cat_iMatch, data = Dsub, random =  list(~1|ppn, ~1|target), 
              method = "ML", na.action = na.exclude)
...
length(unique(Dsub[Dsub$stressdiff_cat == "L1 precedes" & 
                     Dsub$stressed.syllable.L2==Dsub$Nsyllables_correct,]$target))
unique(Dsub[Dsub$stressdiff_cat == "L1 precedes" & 
              Dsub$stressed.syllable.L2==Dsub$Nsyllables_correct,]$target)

Dsub_excl <- Dsub
Dsub_excl$excludeditems <- 0
Dsub_excl[Dsub_excl$stressdiff_cat == "L1 follows" & 
            Dsub_excl$stressed.syllable.L2==1,]$excludeditems <- 1
Dsub_excl[Dsub_excl$stressdiff_cat == "L1 precedes" & 
            Dsub_excl$stressed.syllable.L2==Dsub_excl$Nsyllables_correct,]$excludeditems <- 1
Dsub_excl <- Dsub_excl[Dsub_excl$excludeditems!=1,]

model0_excl <- lme(asynchrony~1, data = Dsub_excl, random =  list(~1|ppn, ~1|target), 
                   method = "ML", na.action = na.exclude)

# make sure the 'L1_L2 match' is mapped onto the intercept
Dsub_excl$stressdiff_cat_iMatch <- relevel(as.factor(Dsub_excl$stressdiff_cat),"L1_L2 match")

model3_excl <- lme(asynchrony~stressdiff_cat_iMatch, data = Dsub_excl, random =  list(~1|ppn, ~1|target), 
                   method = "ML", na.action = na.exclude)
...
```{r, echo=F, warning=F, fig.cap= "Raw gesture-speech (a)synchrony in stress-mismatching and stress-matching items."}
...
model0 <- lme(asynchrony~1, data = Dsub_l2attr, random = list(~1|ppn, ~1|target), 
              method = "ML", na.action = na.exclude)

model1 <- lme(asynchrony~stressdiff_cat_iMatch, data = Dsub_l2attr, random =  list(~1|ppn, ~1|target), 
              method = "ML", na.action = na.exclude)
print(anova(model0, model1)) #test difference basemodel versus model 1
...
```{r, echo = F, fig.cap="Scatterplot of individual native language attraction as a function of foreign language proficiency."}
...
### ExplRQ.2: 'acoustics-neutral' analysis of kinematic accent

As another exploratory analysis, we asked whether the 'kinematic accent' would also be statistically detectable and reliable if the analysis (e.g., run in ConfRQ.2A) were ignorant to the actually produced acoustics produced by the learners.

That is, the analysis in ConfRQ.2A is dependent on our acoustically-defined stress score. It uses this score to detect which syllable was acoustically most prominent in the learners' speech. Then, it takes this acoustically most prominent syllable and calculates the gesture synchrony relative to this detected syllable, as pre-registered.

However, one may also run an analysis taking the syllable that is phonologically stressed in Spanish as the speech anchor point, irrespective of the actually produced acoustics. For instance, in a word like "profeSOR" with word-final stress in Spanish (note: in Dutch, it's "proFESsor" with stress on -FES-), we would predict to find *more early gestures* in Dutch learners' productions, compared to the stress-matching control condition (e.g., SP "bailaRIna" - NL "balleRIna"). Conversely, in a word like "MAchina" with word-initial stress in Spanish (note: in Dutch, it's "maCHIne" with stress on -CHI-), we would predict to find *more late gestures* in Dutch learners' productions, compared to stress-matching control. Note that, because this analysis is independent from the stress score, we include all gesture trials.

First, we plotted the gesture-speech asynchrony across all gesture trials, this time taking the phonologically stressed syllable (i.e., the "foreign language target syllable") as anchor point (set at 0; see Figure 9).

Figure 9:

```{r, echo = F, fig.cap= "Raw gesture-speech (a)synchrony relative to the phonologically stressed syllable in stress-mismatching and stress-matching items."}
# now for phon anchor
D$asynchrony_phon <- D$time_beat - D$time_syl_L2
Dsub_phon <- subset(D, (condition == "gesture"))
Dsub1_phon <- subset(D, (condition == "gesture") & stressdiff_cat=='L1 precedes')
a_phon <- ggplot(Dsub1_phon, aes(x = stress, y= asynchrony_phon))+geom_hline(yintercept = 0, color = "red", size = 0.5)+geom_violin(fill=NA) + geom_quasirandom(color = "black", size = 0.7, alpha = 0.5)+geom_boxplot(alpha = 0)+theme_bw() + coord_flip()+ylim(-730, 600)
Dsub2_phon <- subset(D, (condition == "gesture") & stressdiff_cat=='L1 follows')
b_phon <- ggplot(Dsub2_phon, aes(x = stress, y= asynchrony_phon))+geom_hline(yintercept = 0, color = "red", size = 0.5)+geom_violin(fill=NA) + geom_quasirandom(color = "black", size = 0.7, alpha = 0.5)+geom_boxplot(alpha = 0)+theme_bw() + coord_flip()+ylim(-730, 600)
Dsub3_phon <- subset(D, (condition == "gesture") & stressdiff_cat=='L1_L2 match')
c_phon <- ggplot(Dsub3_phon, aes(x = stress, y= asynchrony_phon))+geom_hline(yintercept = 0, color = "red", size = 0.5)+geom_violin(fill=NA) + geom_quasirandom(color = "black", size = 0.7, alpha = 0.5)+geom_boxplot(alpha = 0)+theme_bw() + coord_flip()+ylim(-730, 600)

plot_grid(a_phon, b_phon, c_phon, ncol = 1, align = "v")
```

When comparing this Figure 9 to Figure 6 we see very similar patterns: the top panel seems to have a more negative gesture-speech (a)synchrony than the stress-matching control bottom panel. Conversely, the middle panel seems to have more positive gesture-speech (a)synchrony – relative to the stress-matching control data. These data were statistically tested using a similar model as used for ConfRQ.2A.

```{r, echo = T}
D$asynchrony_phon <- D$time_beat - D$time_syl_L2
Dsub_phon <- subset(D, (condition == "gesture"))

model0_phon <- lme(asynchrony_phon~1, data = Dsub_phon, random =  list(~1|ppn, ~1|target), 
                   method = "ML", na.action = na.exclude)

# make sure the 'L1_L2 match' is mapped onto the intercept
Dsub_phon$stressdiff_cat_iMatch <- relevel(as.factor(Dsub_phon$stressdiff_cat),"L1_L2 match")

model3_phon <- lme(asynchrony_phon~stressdiff_cat_iMatch, data = Dsub_phon, random =  list(~1|ppn, ~1|target), 
                   method = "ML", na.action = na.exclude)
print(anova(model0_phon,model3_phon))
print(summary(model3_phon))
```

This model confirms that gestures fell significantly later in time in trials where the native competitor syllable followed the foreign language target syllable. Conversely, gestures fell significantly earlier in time in trials where the native competitor syllable preceded the foreign language target syllable. Therefore, even when analyzing our data without making use of our stress score, we find evidence for a kinematic accent in learners' productions.

Finally, note that the present exploratory analysis included all trials with a gesture, without considering how our learners pronounced the words. One might have argued that this exploratory analysis should not have found any evidence for a kinematic accent because the kinematic accent had opposite directions in 'correct' (ConfRQ.2A) vs. 'incorrect' productions (ConfRQ.2B). Perhaps that should have cancelled out any effect when considering all trials with a gesture, irrespective of the acoustics. However, one should consider that the data used in ConfRQ.2A included 53% of all stress-mismatching cognate trials, while ConfRQ.2B only included 27% of all stress-mismatching cognate trials. This accounts for why the present exploratory analysis is qualitatively similar to ConfRQ.2A.

## Reproducibility

Below are the specific package versions we have used for our analyses.

```{r}
# Save session info to a file
sink("session_info_SIM_SI.txt")
sessionInfo()
```

## References

All analyses were conducted in R [@R-base]. Data visualization was performed using ggplot2 [@R-ggplot2] and plotly [@R-plotly]. Mixed-effects models were fitted using lme4 [@R-lme4] and nlme [@R-nlme]. Post-hoc comparisons used lsmeans [@R-lsmeans]. Data manipulation was performed with plyr [@R-plyr], tidyr [@R-tidyr], and stringr [@R-stringr]. Additional packages included RColorBrewer [@R-RColorBrewer], wesanderson [@R-wesanderson], ggbeeswarm [@R-ggbeeswarm], papaja [@R-papaja], simr [@R-simr], gridExtra [@R-gridExtra], readr [@R-readr], and cowplot [@R-cowplot].
