---
title: "Extended Results of 'Foreign Language Learners Show a Kinematic Accent in Their Co-speech Hand Movements'"
author: "**Hans Rutger Bosker, Marieke Hoetjes, Doenja Hustin, Wim Pouw, Lieke van Maastricht** (shared first authorship among all authors)"
date: "*Jan 19, 2024*"
output: 
  html_document:
    toc: true
...
model0_phon <- lme(asynchrony_phon~1, data = Dsub_phon, random =  list(~1|ppn, ~1|target), method = "ML", na.action = na.exclude)

# make sure the 'L1_L2 match' is mapped onto the intercept
Dsub_phon$stressdiff_cat_iMatch <- relevel(as.factor(Dsub_phon$stressdiff_cat),"L1_L2 match")

model3_phon <- lme(asynchrony_phon~stressdiff_cat_iMatch, data = Dsub_phon, random =  list(~1|ppn, ~1|target), method = "ML", na.action = na.exclude)
